...
from app.core.structured_logger import setup_json_logging, log_request
import os
...
async def log_requests(request: Request, call_next):
    ...
    try:
        ...
        log_request(corr_id, request.method, str(request.url.path), response.status_code, duration_ms)
        
        # Development: Pretty logs  
        if os.getenv("ENVIRONMENT") != "production":
            print(f"✅ [{corr_id}] {request.method} {request.url.path} - {response.status_code} ({duration:.3f}s)")
        ...
        # Development: Pretty logs
        if os.getenv("ENVIRONMENT") != "production":
            print(f"💥 [{corr_id}] {request.method} {request.url.path} - ERROR: {str(e)} ({duration:.3f}s)")
