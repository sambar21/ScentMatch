...
def validate_filename(filename: str) -> str:
    ...
    if not filename or len(filename) > 255:
        raise ValidationError("Invalid filename length", "filename", get_correlation_id())
    
    # Remove dangerous characters
    dangerous_chars = ['/', '\\', '..', '<', '>', ':', '"', '|', '?', '*']
    for char in dangerous_chars:
        if char in filename:
            raise ValidationError(f"Filename contains invalid character: {char}", "filename", get_correlation_id())
...
def sanitize_text_input(text: str, max_length: int = 1000) -> str:
    ...
    if len(text) > max_length:
        raise ValidationError(f"Text too long (max {max_length} characters)", "text", get_correlation_id())
...
def validate_uuid(uuid_str: str, field_name: str = "id") -> str:
    ...
    if not re.match(uuid_pattern, uuid_str.lower()):
        raise ValidationError(f"Invalid {field_name} format", field_name, get_correlation_id())
